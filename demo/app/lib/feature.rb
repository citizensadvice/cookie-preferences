...
class Feature
  ...
  def self.explicitly_disabled?(flag_name)
    # for when a flag defaults to true, so only explicitly setting it to false will disable it
    ActiveModel::Type::Boolean.new.cast(ENV.fetch(flag_name, nil)) == false
  end
end
